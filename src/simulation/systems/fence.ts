...
export function updateFences(state: SimulationState): void {
  ...
  const decayInterval = getParamNumber('fenceIntegrityDecayIntervalTicks');
  const decayAmount = getParamNumber('fenceIntegrityDecayAmount');
  const aggThreshold = getParamNumber('fenceHighAggressionThreshold');
  ...
  for (const fence of state.fences) {
    if (
      ...
      fence.integrity = Math.max(0, fence.integrity - decayAmount);
    }

    if (storm) {
      fence.stress = Math.min(100, fence.stress + getParamNumber('fenceStormStressPerTick'));
    }

    const zoneDinos = state.dinosaurs.filter((d) => d.zoneId === fence.zoneId);
    for (const dino of zoneDinos) {
      if (dino.aggression >= aggThreshold && dino.aiState !== 'Idle') {
        fence.stress = Math.min(
          100,
          fence.stress + getParamNumber('fenceStressHighAggression'),
        );
      }
      if (dino.aiState === 'FenceTesting' && dino.targetFenceId === fence.id) {
        fence.stress = Math.min(
          100,
          fence.stress + getParamNumber('fenceStressFenceTesting'),
        );
      }
    }
